...
import com.example.thymeleftTest.dto.IveMember;
...
public class ThymeleafController {
    ...
    public String variable(Model model) {
        ...
        IveMember iveMember = new IveMember("이서",23,"제주도");
        memberList.add(iveMember);
        model.addAttribute("list",memberList);

        //Map으로 보내기
        Map<String, IveMember> map = new HashMap<>();
        IveMember hong = new IveMember("홍길동",30, "울릉도");
        map.put("h", hong);

        IveMember park = new IveMember("박보검", 35, "인천");
        map.put("p", park);
